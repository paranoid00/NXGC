# 实验一
## 实验目的
- [x] 把上面```https://www.exploit-db.com/shellcodes/48116```链接中shellcode能够成功运行
- [x] 能从汇编源码编译通过并成功dump
...
- 修改第二部分汇编代码，运行得到二进制代码
![](image/notepad.png)
![](image/Notepad-code.png)
![](image/notepad-result.png)

### 通过c语言编程使用原生APT UrlDownloadToFileA下载可执行文件project.exe
- 搭建本地建议局域网`python -m http.server`
- 打开`localhost：8000`可以看见本地的文件
- 将project2.exe下载到本地，运行即可打开记事本
![](image/downloadtofile.png)
![](image/last.jpg)


## 参考文件
- [在使用LoadLibrary( )函数动态链接DLL文件时出现的问题的解决方法](https://www.cnblogs.com/xxn-180727/p/9378519.html)
